...
class BeatGANsEncoderModel(nn.Module):
    ...
    def forward(self, x, t=None, return_2d_feature=False):
        ...
        h = self.out(h)
        # add noise
        # snr = th.tensor(random.choices(self.snr_list, k=h.shape[0])).cuda()
        
        snr = th.tensor(10).cuda()
        h_power = h.pow(2).mean(dim=1)
        noise_power = h_power / (10 ** (snr / 10))
        noise = th.randn_like(h) * (th.sqrt(noise_power).unsqueeze(1))
        h = h + noise
