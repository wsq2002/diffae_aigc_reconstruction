...
start_epoch = 0
# resume
if os.path.exists(f'checkpoints/ffhq128_denoising/last.ckpt'):
    ckpt = torch.load(f'checkpoints/ffhq128_denoising/last.ckpt', map_location='cpu')
...
if mode == 'train':
    for epoch in range(start_epoch, max_epochs):
        with tqdm(dataloader, desc=f"Epoch {epoch}") as pbar:
            ...
            torch.save(ckpt, f'checkpoints/ffhq128_denoising/last.ckpt')
else:
    diffae = diffae.cuda()
    diffae = diffae.eval()
    with torch.no_grad():
        transform = transforms.Compose([transforms.Resize(128), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
        img = Image.open("/home/cg/semcom/diff_auto/datasets/celebahq/CelebAMask-HQ/CelebA-HQ-img/0.jpg")
        img = transform(img).cuda().unsqueeze(0)
        latents = diffae.encode(img)
        latents_norm = diffae.normalize(latents)
        # add noise
        snr = torch.tensor(random.choices(SNR_list, k=latents.shape[0])).cuda()
        latents_power = latents_norm.pow(2).mean(dim=1)
