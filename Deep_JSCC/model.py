...
from Deep_JSCC.my_channel import Channel
...
class _Encoder(nn.Module):
    def __init__(self, c=1, is_temp=False, P=1):
        ...
        self.conv1 = _ConvWithPReLU(in_channels=3, out_channels=64, kernel_size=3, stride=2, padding=1)
        self.conv2 = _ConvWithPReLU(in_channels=64, out_channels=128, kernel_size=3, stride=2, padding=1)
        self.conv3 = _ConvWithPReLU(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.conv4 = _ConvWithPReLU(in_channels=128, out_channels=128, kernel_size=3, padding=1)
        self.conv5 = _ConvWithPReLU(in_channels=128, out_channels=2*c, kernel_size=3, padding=1)
...
class _Decoder(nn.Module):
    def __init__(self, c=1):
        ...
        self.tconv1 = _TransConvWithPReLU(
            in_channels=2*c, out_channels=128, kernel_size=3, stride=1, padding=1)
        ...
        self.tconv5 = _TransConvWithPReLU(
            in_channels=64, out_channels=3, kernel_size=3, stride=2, padding=1, output_padding=1, activate=nn.Sigmoid())
        # self.tconv5 = _TransConvWithPReLU(
        #     in_channels=16, out_channels=3, kernel_size=6, stride=2, padding=2, output_padding=1,activate=nn.Sigmoid())
...
class DeepJSCC(nn.Module):
    ...
    def forward(self, x):
        z = self.encoder(x)
        # 现在 Deep JSCC 中没有加噪了，加噪的逻辑放到类外面了
        # if hasattr(self, 'channel') and self.channel is not None:
        # # train
        #     # snr = choice(self.snrs)   
        #     # snr = 5
        #     z = self.channel.forward(z)
        # x_hat = self.decoder(z)
        x_hat = self.decoder(z)
        return x_hat
    
    def encoder_forward_channel(self, x, snr=None):
        z = self.encoder(x)
        if hasattr(self, 'channel') and self.channel is not None:
            z = self.channel.forward(z, snr)
        return z
