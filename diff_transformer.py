...
class DiffTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, num_heads, num_layers, max_seq_length=512):
        ...
        self.snr_dict = dict(zip(list(range(5, 21, 5)), list(range(4))))
    ...
    def forward(self, x, snr):
        ...
        # X = self.token_emb(snr).unsqueeze(1) + self.pos_emb(positions)  # (batch, N, d_model)
        X = self.token_emb(snr) + x  # (batch, 2, d_model)
        X = X.unsqueeze(1)
