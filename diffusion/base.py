    ...
    def p_mean_variance(self,
        ...
        with autocast(self.conf.fp16):
            model_forward = model.forward(x=x,
                                          t=self._scale_timesteps(t),
                                          **model_kwargs)
        model_output = model_forward.pred  # model_output即为预测出来的nosie，eps
        ...
        return {
            ...
            'model_forward': model_forward, # eps
        }

    def _predict_xstart_from_eps(self, x_t, t, eps):  # 根据x_t = sqrt(alpha_bar)*x_0 + sqrt(1-alpha_bar)*eps, 计算x_0
